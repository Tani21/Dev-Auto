...
elif [ -z ${username:+word} ]
then
        echo "ERROR: username is not set" >&2; exit 1;
elif [ -z ${password:+word} ]
then
...
if ! (crontab -l | grep -Fxq "0 */12 * * * $path_to_script $credentials_file $backup_directory")
then
        crontab -l | { cat; echo "0 */12 * * * $path_to_script $credentials_file $backup_directory"; } | crontab - echo "Script added to Cron"
fi
